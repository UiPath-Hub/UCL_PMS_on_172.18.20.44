export default {
	...
	search:async()=>{
		...
		if(await this.loadTable("PMS_INQUIRY_HEADER_LM")){
			console.log("Processed data",TableDisplay["PMS_INQUIRY_HEADER_LM"].data);
		}else{
			TableDisplay["PMS_INQUIRY_HEADER_LM"].data = []
		}
		resetWidget(PMS_INQUIRY_HEADER_LM.widgetName);
	},
	onEditItemClick:()=>{
		if(!Init.permissionsCheck(Configs.permissions.VIEW,false))return;
