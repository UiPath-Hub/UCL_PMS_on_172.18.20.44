export default {
	V:3,
	LastChanged:"definded comment : InitializationDataList = [{ENTITY:Appsmith JS Object, DATA: {propName:any}}]",
	...
	manualValidateV2:async (defaultEntities,widgetsMap)=>{
		...
		await Promise.all( Object.keys(defaultEntities).map(async (key)=>{
			...
			if (!widgetsMap[keystr].isValid && !widgetsMap[keystr].isDisabled && widgetsMap[keystr].isVisible) {
				if(defaultEntities[widgetName].color!=Configs.requiredColorAlert){
					defaultEntities[widgetName].color = Configs.requiredColorAlert;
				}
				alert.push(keystr);
			} else {
				if(defaultEntities[widgetName].color!=Configs.requiredColorPass){
					defaultEntities[widgetName].color = Configs.requiredColorPass;
				}
			}
