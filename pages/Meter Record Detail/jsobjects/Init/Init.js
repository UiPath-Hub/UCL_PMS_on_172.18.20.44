export default {
	...
	pageLoad:async ()=>{
		...
		if(await this.sessionCheck()){
			if(await this.permissionsCheck(Configs.permissions.VIEW,true)){
				...
				resetWidget("CONTAINER_ALL",true);
			}
	...
	initDefault:async ()=>{
		const setAttribute =async (data)=>{
			if(data != undefined){
				Object.keys(Default_METER).map((attribute)=>{
					if(data[attribute] != undefined){
						Default_METER[attribute].data = data[attribute];
					}
				});			
				//await resetWidget("Form1",true);
			}
		}
		await SELECT_METER_DETAIL.run();
		if(SELECT_METER_DETAIL.data==undefined)return;
		let data = SELECT_METER_DETAIL.data[0];
