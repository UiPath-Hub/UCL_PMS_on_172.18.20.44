export default {
	...
	onBttn_DUPLICATE_INVOICE:async(confirm)=>{
		...
		if(confirm){
			_4_DraftInvoice.run().then(async() => {
				...
				if(SELECT_RECEIPT.data[0]?.INVOICE_NO){
					await closeModal(MODAL_DUPLICATE_CONFIRM.name);
	...
	onConfirmEditItem: async(newDescription)=>{
		let editID = Table_PMS_RECEIPT_DETAIL_LM.selectedRows.map(ele=>ele.INVOICE_DETAIL_ID);
		...
		await resetWidget(Table_PMS_RECEIPT_DETAIL_LM.widgetName,false);
