...
dataDisplayStartTime:moment("2021-01-01","YYYY-MM-DD"), //moment.tz("Asia/Bangkok").format("yyyy-mm-dd"),
  pageLoad	:async ()=>{
		...
		if(await this.sessionCheck()){
			if(await this.permissionsCheck()){
	...
	permissionsCheck:()=>{
		...
		if(! PERMISSIONS.includes(Configs.permissions.VIEW)){
			Configs.errorAlert = "You do not have a permission to View this page. Please contact admin."
